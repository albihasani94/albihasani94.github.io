...
What is Helidon? Turning to ethimology, Helidon comes from Greece and it means the _swallow_ bird in English.
...
Helidon SE embraces the latest Java SE features: reactive streams, asynchronous and functional programming, and fluent-style APIs. At the same time, Helidon MP provides an implementation to the MicroProfile specification. This is the child of Java EE and the cloud.
...
As the buld tool fetches the dependencies, runs some tests, we will be set.
...
Secondly, but not less important, there is a clear separation between the business logic and the libraries, as it should be.
...
On a personal note, it feels refreshing to get to write after such a long time. Since the last time I did Earth has seen a global disease outbreak, the quarantine, UFOs were confirmed by the US goverment, and the protests in USA. We have to come together as humans, be kind, and this too shall pass.
