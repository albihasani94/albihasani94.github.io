...
Designing programs is hard. Designing languages that make you able to write this programs in a safe way is harder.
...
You hate it. Now matter how much experience you have under your belt, you always fail to the same mistake. It's like that bad dream that keeps repeating. NullPointerException is the monster. 
...
As a matter of fact, we will see one right now.
...
Looking more carefully, beside following the flow of the iterations and debugging, which could get complex in a short time, we do not know which one of the references in the stream caused it.
...
Making it so easy, the developers can focus on actually fixing the NPE, rather than starting a quest to find the reference that caused it. In most cases, this feature will serve it for us. Going back to the statement from Hoare, the Java language designers take it seriously to offer the best programming experience to developers, if not for reversing decisions of the past which could not be abandoned due to the backwards-compatibility promise, instead offering a bag of improvements such as this one.
